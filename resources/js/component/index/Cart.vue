    ...
    <div class="container">
      ...
      <div v-else class="row">
        ...
        <div class="col-lg-5 ms-auto my-4">
          ...
          <button class="btn theme-login btn-lg mt-3 text-light rounded-pill w-100" @click="clearCart" :disabled="processing">
            Clear Cart
          </button>
        </div>
      </div>
...
export default {
  ...
  setup() {
    ...
    const clearCart = async () => {
      ...
      try {
        const { data } = await axios.delete('/api/cart', authHeader())
        applyCartResponse(data)
      } catch (error) {
        errorMessage.value = error.response?.data?.message || 'Unable to clear cart.'
      } finally {
